          ...
          <div class="table-responsive mt-4">
            <table class="table table-bordered table-striped text-center">
              ...
              <?php if($data) { $no=1;foreach($data as $row) {
                  ...
                  <td>'.$row['nama'].'</td>
                  <td>'.$row['tanggal'].'</td>
                  <td><a href="'.base_url('rontgen/print_file/'.$row['no_pasien'].'/'.$row['id_rontgen']).'"><i class="fa fa-print"></i></a></td>
                  </tr>';
              } } else { echo '<tr><td colspan="5">Data Rontgen Tidak Ditemukan</td></tr>'; } ?>
            </table>
          </div>
        </div>
      </div>
